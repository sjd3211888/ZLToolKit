...
class SQLinstance
{
	...
	vector<SqlPool::Ptr> sqlpoolmap;
	SqlPool::Ptr getpool(int index);
};
SqlPool::Ptr SQLinstance::getpool(int index)
{
	if(index>sqlpoolmap.size()-1)
	{
		return nullptr;
	}
    auto pool = sqlpoolmap[index];
    if(pool)
	{
		return pool;
	}
...
class SqlStream {
public:
    SqlStream(const char *sql,int index=0) : _sql(sql),_index(index){}
    ~SqlStream() {}
    ...
    SqlStream& operator <<(T &&data) {
        ...
        string str = SQLinstance::Instance().getpool(_index)->escape(_str_tmp.str());
...
public:
     ...
     */
    SqlWriter(const char *sql,bool throwAble = true,int index=0) : _sqlstream(sql),_throwAble(throwAble),_index(index) {}
    ~SqlWriter() {}
    ...
    void operator <<(std::ostream&(*f)(std::ostream&)) {
        //异步执行sql不会抛异常
        SQLinstance::Instance().getpool(_index)->asyncQuery((string)_sqlstream);
    }
    ...
    int64_t operator <<(vector<Row> &ret) {
        try {
            _affectedRows = SQLinstance::Instance().getpool(_index)->syncQuery(_rowId,ret, (string)_sqlstream);
        }catch (std::exception &ex){
            if(!_throwAble){
                WarnL << ex.what()<<"sql index is "<<_index;
            } else {
                throw;
            }
        }
